...
const PaymentModal = ({ isOpen, onClose, lesson, onPaymentSuccess }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Current: Direct API call to backend simulation
      // Future: Handle different payment methods and redirects
      ...
      // TODO: Add card details for real card processing
      if (paymentMethod === 'card') {
        paymentData.cardDetails = cardDetails;
      }
      
      const response = await axiosInstance.post('/user/purchase/lesson', paymentData);
      ...
      if (response.data.status === 'success') {
        ...
        // TODO: Handle payment gateway redirects
        // For VNPay, MoMo, ZaloPay - redirect to payment URL
        window.location.href = response.data.paymentUrl;
      }
      
      /*
      // FUTURE: Handle different payment gateway responses
      switch (paymentMethod) {
        ...
        case 'banking':
          ...
          toast.info('Vui lòng chuyển khoản theo thông tin đã cung cấp.');
          break;
      }
      */
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gradient-to-r from-purple-100 to-blue-100 rounded-xl p-4 mb-6">
          ...
          <div className="text-sm text-gray-500 mt-2">
            ✅ Truy cập không giới hạn trong 30 ngày
          </div>
        </div>
